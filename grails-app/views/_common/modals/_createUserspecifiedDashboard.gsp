<div id="CreateUserspecifiedDashboardModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="CreateUserspecifiedDashboardModalLabel">
  <div class="modal-header">
    ...
    <h4 id="CreateUserspecifiedDashboardModalLabel">
        <g:message code="de.iteratec.isocsi.dashBoardControllers.custom.title" default="To save as custom dashboard, please enter additional information!"/>
    </h4>
  </div>
  ...
  <div class="modal-footer">
    <g:form>
        ...
        <a href="#" class="btn btn-primary" onclick="saveCustomDashboard()"><g:message code="de.iteratec.ism.ui.labels.save" default="Save"/></a>
    </g:form>
...
<r:script>
    ...
    function saveCustomDashboard(){
        ...
        if (dashboardName.value.trim() !== "") {
            ...
            startSpinner(document.getElementById('spinner-position'));

            $.ajax({
                type: 'POST',
                url: '${createLink(action: 'validateDashboardName', absolute: true)}',
                data: { proposedDashboardName: dashboardName.value.trim(), noprepend: true },
                statusCode: {
