...
class EventResultServiceTests {
    ...
    void setUp() {
        ...
        for (int i = 0; i < jobs.size(); i++) {
            ...
            MeasuredEvent measuredEvent = new MeasuredEvent(name: "unsuedMeasuredEvent").save(validate: false)
    ...
    private EventResult createEventResult(MeasuredEvent theEventToLookFor, CachedView cachedView, Boolean medianValue, JobResult jobResultWithinDateRange1, Date measuringDateOfWithinDateRange1) {
        new EventResult(
                jobGroup: jobResultWithinDateRange1.job.jobGroup,
                measuredEvent: theEventToLookFor,
                page: theEventToLookFor.testedPage,
                browser: ffBrowser,
                location: ffAgent1,
                cachedView: cachedView, medianValue: medianValue,
                jobResult: jobResultWithinDateRange1, jobResultDate: measuringDateOfWithinDateRange1,
                /*Not of interest but required by constraint: */
                jobResultJobConfigId: 1, wptStatus: 200,
                numberOfWptRun: 1, speedIndex:
                EventResult.SPEED_INDEX_DEFAULT_VALUE,
                connectivityProfile: connectivityProfile).save(failOnError: true);
    }
