...
class WptResultXml {
	...
	Integer getTestStepCount() {
		...
		return version == WptXmlResultVersion.BEFORE_MULTISTEP ?
				responseNode.data.median.isEmpty() ? 0 : 1 :
				responseNode.data.median.firstView.testStep.size()
	...
	Integer getRunNumberOfMedianViewNode(CachedView cachedView, Integer testStepZeroBasedIndex){
		...
		if (cachedView == CachedView.UNCACHED) {
			return version == WptXmlResultVersion.BEFORE_MULTISTEP ?
					responseNode.data.median.firstView.run.getAt(0).toInteger() :		//getAt(0) on run cause of a bug in wpt: duplicated run node
					responseNode.data.median.firstView.testStep.getAt(testStepZeroBasedIndex).run.getAt(0).toInteger()
		}else if (cachedView == CachedView.CACHED) {
			return version == WptXmlResultVersion.BEFORE_MULTISTEP ?
					responseNode.data.median.repeatView.run.getAt(0).toInteger() :
					responseNode.data.median.repeatView.testStep.getAt(testStepZeroBasedIndex).run.getAt(0).toInteger()
		} else {
			throw new IllegalArgumentException("Argument cachedView should be UNCACHED or CACHED, but was ${cachedView}!")
		}
	}
