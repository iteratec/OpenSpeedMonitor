...
@Mock([JobResult, EventResult, HttpArchive, CsiAggregation, MeasuredValueUpdateEvent, BatchActivity])
class DbCleanupServiceSpec {
    ...
    void testDeleteResultsDataBefore() {
        ...
        CsiAggregation measuredValueWithBeforeCleanupDate = new CsiAggregation(started: executionDateBeforeCleanUpDate.toDate()).save(validate: false)
        new MeasuredValueUpdateEvent(measuredValueId: measuredValueWithBeforeCleanupDate.id).save(validate: false)

        CsiAggregation measuredValueWithAfterCleanupDate = new CsiAggregation(started: executionDateAfterCleanUpDate.toDate()).save(validate: false)
        ...
        //delete all {@link JobResult}s, {@link EventResult}s, {@link HttpArchive}s, {@link CsiAggregation}s, {@link MeasuredValueUpdateEvent}s older then one year (12 months)
