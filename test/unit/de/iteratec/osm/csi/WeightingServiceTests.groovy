...
@Mock([EventResult, MeasuredValue, MeasuredValueUpdateEvent])
class WeightingServiceTests {
	...
	void testGetWeightWithBrowserAsWeightFactor() {
		// test specific data
		Set<WeightFactor> weightFactors = [WeightFactor.BROWSER] as Set
	...
	void testGetWeightWithMultipleWeightFactorsForEventResults() {
		// test specific data
		CsiValue eventResultWeightFiftyTwoAm = new EventResult(tag: TAG_INDICATING_WEIGHT_OF_FIFTY_PERCENT, jobResultDate: SHOULD_BE_MAPPED_TO_TWO_A_CLOCK_AM.toDate())
		CsiValue eventResultWeightSeventyFivePm = new EventResult(tag: TAG_INDICATING_WEIGHT_OF_SEVENTY_PERCENT, jobResultDate: SHOULD_BE_MAPPED_TO_FIVE_A_CLOCK_PM.toDate())
		
		//test with all three WeightFactors
		Double deliveredWeight = serviceUnderTest.getWeight(eventResultWeightFiftyTwoAm, [WeightFactor.HOUROFDAY, WeightFactor.PAGE, WeightFactor.BROWSER] as Set)
		double expectedHourofdayWeight = 0.2d
		double expectedBrowserWeight = 0.5d
		double expectedPageWeight = 0.5d
		double expectedWeight = expectedHourofdayWeight * expectedBrowserWeight * expectedPageWeight
		assertEquals(expectedWeight, deliveredWeight,DELTA)
		
		deliveredWeight = serviceUnderTest.getWeight(eventResultWeightSeventyFivePm, [WeightFactor.HOUROFDAY, WeightFactor.PAGE, WeightFactor.BROWSER] as Set)
		expectedHourofdayWeight = 7.3d
		expectedBrowserWeight = 0.7d
		expectedPageWeight = 0.7d
		expectedWeight = expectedHourofdayWeight * expectedBrowserWeight * expectedPageWeight
		assertEquals(expectedWeight, deliveredWeight,DELTA)
		
		//test with two weightFactors
		deliveredWeight = serviceUnderTest.getWeight(eventResultWeightSeventyFivePm, [WeightFactor.PAGE, WeightFactor.BROWSER] as Set)
		expectedBrowserWeight = 0.7d
		expectedPageWeight = 0.7d
		expectedWeight = expectedBrowserWeight * expectedPageWeight
		assertEquals(expectedWeight, deliveredWeight,DELTA)
		
		deliveredWeight = serviceUnderTest.getWeight(eventResultWeightSeventyFivePm, [WeightFactor.HOUROFDAY, WeightFactor.BROWSER] as Set)
		expectedHourofdayWeight = 7.3d
		expectedBrowserWeight = 0.7d
		expectedWeight = expectedHourofdayWeight * expectedBrowserWeight
		assertEquals(expectedWeight, deliveredWeight,DELTA)
	...
	void testGetWeightedCsiValuesFromEventResults(){
		...
		CsiValue eventResultWeightFiftyTwoAm = new EventResult(
			...
			docCompleteTimeInMillisecs: 1000).save(validate: false)
		...
		List<WeightedCsiValue> weightedValues = serviceUnderTest.getWeightedCsiValues(
			...
			[WeightFactor.HOUROFDAY, WeightFactor.PAGE, WeightFactor.BROWSER] as Set)
		...
		//tests with browser and page as WeightFactors
		
		weightedValues = serviceUnderTest.getWeightedCsiValues(
			[eventResultWeightFiftyTwoAm, eventResultWeightSeventyFivePm],
			[WeightFactor.BROWSER, WeightFactor.PAGE] as Set)
		
		assertEquals(2, weightedValues.size())
		
		expectedBrowserWeight = 0.5d
		expectedPageWeight = 0.5d
		expectedWeight = expectedBrowserWeight * expectedPageWeight
		assertEquals(1, weightedValues.findAll{
				it.underlyingEventResultIds.equals([1]) && it.weightedValue.getValue().equals(10d) && it.weightedValue.getWeight().equals(expectedWeight)
		...
		//tests with just browser as WeightFactor
		
		weightedValues = serviceUnderTest.getWeightedCsiValues(
			[eventResultWeightFiftyTwoAm, eventResultWeightSeventyFivePm],
			[WeightFactor.BROWSER] as Set)
		
		assertEquals(2, weightedValues.size())
		
		expectedBrowserWeight = 0.5d
		assertEquals(1, weightedValues.findAll{
				it.underlyingEventResultIds.equals([1]) && it.weightedValue.getValue().equals(10d) && it.weightedValue.getWeight().equals(expectedBrowserWeight)
			}.size())
		
		expectedBrowserWeight = 0.7d
		assertEquals(1, weightedValues.findAll{
			it.underlyingEventResultIds.equals([2]) && it.weightedValue.getValue().equals(20d) && it.weightedValue.getWeight().equals(expectedBrowserWeight)
		}.size())
	...
	void testGetWeightedCsiValuesFromMeasuredValues(){
		...
		expectedBrowserWeight = 0.5d
		assertEquals(1, weightedValues.findAll{
				it.underlyingEventResultIds.equals([1l, 2l, 3l]) && it.weightedValue.getValue().equals(10d) && it.weightedValue.getWeight().equals(expectedBrowserWeight)
			}.size())
		
		expectedBrowserWeight = 0.7d
		assertEquals(1, weightedValues.findAll{
			it.underlyingEventResultIds.equals([4l, 5l, 6l]) && it.weightedValue.getValue().equals(20d) && it.weightedValue.getWeight().equals(expectedBrowserWeight)
		}.size())
	...
	void testFlattenWeightedCsiValuesCalledWithData(){
		...
		List<WeightedCsiValue> ofFirstWeight = flattened.findAll{ Math.abs(it.weightedValue.weight - firstWeight) < DELTA }
		...
		List<WeightedCsiValue> ofSecondWeight = flattened.findAll{ Math.abs(it.weightedValue.weight - secondWeight) < DELTA }
		...
		List<WeightedCsiValue> ofThirdWeight = flattened.findAll{ Math.abs(it.weightedValue.weight - thirdWeight) < DELTA }
